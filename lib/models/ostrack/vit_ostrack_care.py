...
class VisionTransformerOSTrack(BaseBackbone):
    """ Vision Transformer
    ...
    def forward_features(self, z, x):
        ...
        z_v_0 = self.patch_embed(z[0][0]) # b, 64, 768
        z_v_1 = self.patch_embed(z[0][1]) 
        z_i_0 = self.patch_embed(z[1][0])
        z_i_1 = self.patch_embed(z[1][2])
        ...
        x_v += self.pos_embed_x # b, 256, 768
        ...
        z_v = combine_tokens(z_v_0, z_v_1, mode=self.cat_mode)
        z_i = combine_tokens(z_i_0, z_i_1, mode=self.cat_mode)
        ...
        # add z branch
        z = combine_tokens(z_v, z_i, mode=self.cat_mode)
        z = self.pos_drop(z)
        ...
        for i, blk in enumerate(self.blocks):
            # add z branch
            z = blk(z)
            x_v = combine_tokens(z[:, :128, :], x_v[:, 128:, :], mode=self.cat_mode)
            x_i = combine_tokens(z[:, 128:, :], x_i[:, 128:, :], mode=self.cat_mode)
            # original
            x_v = blk(x_v)
            x_i = blk(x_i)
...
@torch.no_grad()
def _load_weights(model: VisionTransformerOSTrack, checkpoint_path: str, prefix: str = ''):
...
def _create_vision_transformer(variant, pretrained=False, default_cfg=None, **kwargs):
    ...
    model = VisionTransformerOSTrack(**kwargs)
