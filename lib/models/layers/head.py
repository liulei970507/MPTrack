...
class MLP(nn.Module):
    ...
    def forward(self, x):
        x = self.gap(x).view(x.size(0), -1) # add gap 
...
class MLPv2(nn.Module):
    ...
    def forward(self, x, box):
        x = self.gap(x).view(x.size(0), -1) # add gap 
        x = torch.cat([x, box.squeeze(1)],dim=1)
        for i, layer in enumerate(self.layers):
            x = F.relu(layer(x)) if i < self.num_layers - 1 else layer(x)
        return x
...
class MLPv3(nn.Module):
    ...
    def forward(self, x, box, rsp):
        x = self.gap(x).view(x.size(0), -1) # add gap 
        x = torch.cat([x, box.squeeze(1)],dim=1)
        # import pdb
        # pdb.set_trace()
        rsp = self.gap(rsp.squeeze(1)).view(rsp.size(0), -1)
