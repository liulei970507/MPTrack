...
class Attention_st(nn.Module):
    def __init__(self, dim, mode, num_heads=8, qkv_bias=False, attn_drop=0., proj_drop=0.,
                 rpe=False, z_size=7, x_size=14, lens_z=-1):
        super().__init__()
        self.num_heads = num_heads
        head_dim = dim // num_heads
        ...
        if lens_z == -1:
            ...
            self.lens_z = lens_z
        self.lens_x = 256  # Number of search region tokens
    def forward(self, x, mask=None, return_attention=False, lens_z=-1):
        # x: B, N, C
        # mask: [B, N, ] torch.bool
        B, N, C = x.shape
        lens_z = self.lens_z
...
class Attention_st_encoder(nn.Module):
    def __init__(self, dim, mode, num_heads=8, qkv_bias=False, attn_drop=0., proj_drop=0.,
                 rpe=False, z_size=7, x_size=14, lens_z=-1):
        super().__init__()
        self.num_heads = num_heads
    ...
    def forward(self, x, mask=None, return_attention=False, lens_z=-1):
        ...
        # import pdb
        # pdb.set_trace()
        q = self.q_linear(q)#.reshape(B, N, 3, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)
        k = self.k_linear(k)#.reshape(B, N, 3, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)
        v = self.v_linear(v)#.reshape(B, N, 3, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)
        ...
        if self.mode == 's2t':
            ...
            x = torch.cat([x, orix[:, lens_z:]], dim=1)
        elif self.mode == 's2s':
            x = torch.cat([x, k], dim=1)
