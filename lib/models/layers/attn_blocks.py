...
from lib.models.layers.attn import Attention, Attention_st, Attention_st_encoder
...
class CASTBlock(nn.Module):
    ...
    def __init__(self, dim, num_heads, mode, mlp_ratio=4., qkv_bias=False, drop=0., attn_drop=0.,
                 drop_path=0., act_layer=nn.GELU, norm_layer=nn.LayerNorm, lens_z=-1):
        super().__init__()
        self.norm1 = norm_layer(dim)
        if lens_z == -1:
            self.lens_z = 64  # Number of template tokens
        else:
            self.lens_z = lens_z
        self.attn_reshape = Attention_st(dim, mode, num_heads=num_heads, qkv_bias=qkv_bias, attn_drop=attn_drop, proj_drop=drop, lens_z=self.lens_z)
        # NOTE: drop path for stochastic depth, we shall see if this is better than dropout here
        self.drop_path = DropPath(drop_path) if drop_path > 0. else nn.Identity()
        self.norm2 = norm_layer(dim)
        mlp_hidden_dim = int(dim * mlp_ratio)
...
class CASTBlock_encoder(nn.Module):
    ...
    def __init__(self, dim, num_heads, mode, mlp_ratio=4., qkv_bias=False, drop=0., attn_drop=0.,
        ...
        if lens_z == -1:
            ...
            self.lens_z = lens_z
        self.attn_reshape = Attention_st_encoder(dim, mode, num_heads=num_heads, qkv_bias=qkv_bias, attn_drop=attn_drop, proj_drop=drop, lens_z=self.lens_z)
        # NOTE: drop path for stochastic depth, we shall see if this is better than dropout here
        self.drop_path = DropPath(drop_path) if drop_path > 0. else nn.Identity()
        self.norm2 = norm_layer(dim)
        mlp_hidden_dim = int(dim * mlp_ratio)
...
class CASTBlock_encoder_womlp(nn.Module):
