...
class UAV_RGBT(BaseVideoDataset):
    ...
    def __init__(self, root=None, image_loader=opencv_loader, split=None, modality = "RGBT"):
        ...
        root = '/mnt/dataShare/Teacher/liulei/Dataset//VTUAV'
        # root = os.path.join(env_settings().UAV_RGBT_dir, 'train') if root is None else root
        ...
        if split is not None:
            if split == 'train':
                ...
                file_path = os.path.join(root, 'test_ST')
                # file_path = os.path.join(root, 'train')
                # file_path = '/DATA/liqi/VTUAV/test_ST'
            elif split == 'val_lt':
                file_path = os.path.join(root, 'test_LT')
            else:
    ...
    def get_frames(self, seq_id, frame_ids, anno=None):
        ...
        if self.modality in ['RGB']:
            ...
            # frame_list = [np.concatenate((frame_list_v[:], frame_list_i[:]), 2)]
            frame_list = []
            for i in range(len(frame_ids)):
                frame_list.append(np.concatenate((frame_list_v[i], frame_list_i[i]), 2))
