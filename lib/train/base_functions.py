...
# datasets related
from lib.train.dataset import Lasot, Got10k, MSCOCOSeq, ImagenetVID, TrackingNet, LasHeR, UAV_RGBT
...
def names2datasets(name_list: list, settings, image_loader):
    ...
    for name in name_list:
        assert name in ["LASOT", "GOT10K_vottrain", "GOT10K_votval", "GOT10K_train_full", "GOT10K_official_val",
                        "COCO17", "VID", "TRACKINGNET", "LasHeR_train",  "LasHeR_test", 'UAV_RGBT_train', 'UAV_RGBT_test']
        if name == "LASOT":
        ...
        if name == "UAV_RGBT_train":
            datasets.append(UAV_RGBT(settings.env.UAV_RGBT_dir, split='train', image_loader=image_loader))
        if name == "UAV_RGBT_test":
            datasets.append(UAV_RGBT(settings.env.UAV_RGBT_dir, split='val_st', image_loader=image_loader))
...
def build_dataloaders(cfg, settings):
    ...
    settings.num_template = getattr(cfg.DATA.TEMPLATE, "NUMBER", 2)
...
def get_optimizer_scheduler(net, cfg):
    ...
    if train_cls:
        ...
        param_dicts = [
            {"params": [p for n, p in net.named_parameters() if ("backbone" not in n or 'tbsi' in n) and p.requires_grad]},
            {
                "params": [p for n, p in net.named_parameters() if "backbone" in n and 'tbsi' not in n and p.requires_grad],
